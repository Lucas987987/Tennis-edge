...
LIMITE ASSUMÉE (27/08/2026, audit v3 §Q) : ce script recalcule won/pnl mais
PAS clv_vs_median (ajouté le même jour dans settle_trade()) -- celui-ci a
besoin des courbes multi-books (`bk`), que ce script, volontairement simple
(il ne lit que set_results.json), n'a jamais chargées. Les trades déjà
réglés par ce script n'auront donc pas de clv_vs_median rétroactif ; les
NOUVEAUX trades traités par le pipeline normal (paper_journal.py -> main())
l'auront. Étendre ce script pour charger les courbes serait un vrai
changement de périmètre, pas une correction -- documenté plutôt que bricolé
sous pression.
"""
import json
import os
