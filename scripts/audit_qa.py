...
TOURNAMENTS_FILE = 'matches_oddspapi.json'
HIST_META_FILE = 'hist_curves_meta.json'
HIST_STALE_HOURS = int(os.environ.get('AUDIT_HIST_STALE_H', '30'))   # run nocturne = 1x/24h + marge
HIST_PARTITION_WARN_MB = int(os.environ.get('AUDIT_HIST_WARN_MB', '80'))  # marge avant la limite dure 100 Mo GitHub
TOURNAMENTS_STALE_HOURS = int(os.environ.get('AUDIT_TOURNAMENTS_STALE_H', '20'))  # au-delà,
# le fichier lu par la page (fetchSports/fetchAllMatches/fetchResults, voir index.html) n'a
# probablement pas été rechargé depuis 1 jour complet -- alerte.
...
def check_tournaments_freshness(problems):
    """matches_oddspapi.json est le fichier lu par index.html (fetchSports,
    fetchAllMatches, fetchResults -- voir la page GitHub Pages) pour charger
    automatiquement les tournois à l'ouverture, plus l'auto-refresh toutes les
    5 min tant que l'onglet est ouvert. Ce check vérifie que le PIPELINE
    (capture_closing) continue bien de le régénérer chaque jour, INDÉPENDAMMENT
    du fait que quelqu'un ait ouvert la page ou non -- l'auto-refresh de la page
    ne peut rafraîchir que ce que le pipeline a effectivement produit.

    IMPORTANT : on lit le TIMESTAMP EMBARQUÉ DANS LE CONTENU du sidecar
    matches_oddspapi_meta.json, PAS le mtime du fichier. Dans un checkout
    GitHub Actions frais, le mtime reflète l'heure du clone (donc toujours
    "maintenant"), pas la dernière vraie écriture -- un check basé dessus
    serait toujours vert à tort, même si le pipeline est arrêté depuis des
    jours. Le sidecar est écrit par capture_closing.py à chaque passage."""
    META_FILE = 'matches_oddspapi_meta.json'
    if not os.path.exists(TOURNAMENTS_FILE):
        problems.append(('CRITIQUE', f"{TOURNAMENTS_FILE} est absent. La page "
                                     f"(fetchSports/fetchAllMatches/fetchResults) n'aura rien à charger."))
        return
    if not os.path.exists(META_FILE):
        problems.append(('INFO', f"{META_FILE} absent -- fraîcheur de {TOURNAMENTS_FILE} non vérifiable "
                                 f"(redéployer capture_closing.py pour l'activer)."))
        return
    try:
        with open(META_FILE, encoding='utf-8') as f:
            meta = json.load(f)
        last_write = datetime.datetime.fromisoformat(meta['last_write'])
        age_h = (_utcnow() - last_write).total_seconds() / 3600
        n_matches = meta.get('n_matches', '?')
    except Exception as e:
        problems.append(('ALERTE', f"{META_FILE} illisible : {e}"))
        return

    if age_h > TOURNAMENTS_STALE_HOURS:
        problems.append(('ALERTE', f"{TOURNAMENTS_FILE} pas régénéré depuis {age_h:.0f}h "
                                   f"(seuil {TOURNAMENTS_STALE_HOURS}h) -- les tournois affichés "
                                   f"sur la page seront périmés même avec l'auto-refresh, puisque "
                                   f"celui-ci ne fait que relire ce fichier."))
    elif n_matches == 0:
        problems.append(('ALERTE', f"{TOURNAMENTS_FILE} à jour ({age_h:.0f}h) mais VIDE "
                                   f"(0 match) -- vérifier fixtures_today()/l'API en amont."))
    else:
        problems.append(('INFO', f"{TOURNAMENTS_FILE} à jour ({age_h:.0f}h, {n_matches} matchs) "
                                 f"-- rechargement quotidien confirmé."))


def check_hist_curves_freshness(problems):
    """Détecte un fetch_book_curves.py qui tourne 'sans erreur' mais dont le
    PUSH échoue silencieusement (`git push || true` avale l'échec) -- c'est
    exactement ce qui s'est produit du 09/08 au 14/08/2026 : book_curves.jsonl
    a atteint 140,73 Mo, GitHub a rejeté chaque push pendant 5 jours sans
    qu'aucune alerte ne se déclenche (voir migrate_hist_partitions.py pour le
    correctif complet -- partitions mensuelles sous parts/hist_*.jsonl).
    Lit hist_curves_meta.json, un sidecar écrit par fetch_book_curves.py à
    CHAQUE run avec un timestamp EMBARQUÉ DANS LE CONTENU (le mtime fichier
    est peu fiable dans un checkout GitHub Actions frais)."""
    if not os.path.exists(HIST_META_FILE):
        problems.append(('INFO', f"{HIST_META_FILE} absent -- fraîcheur de l'historique "
                                 f"(book_curves) non vérifiable (redéployer fetch_book_curves.py)."))
        return
    try:
        with open(HIST_META_FILE, encoding='utf-8') as f:
            meta = json.load(f)
        last_write = datetime.datetime.fromisoformat(meta['last_write'])
        age_h = (_utcnow() - last_write).total_seconds() / 3600
    except Exception as e:
        problems.append(('ALERTE', f"{HIST_META_FILE} illisible : {e}"))
        return

    if age_h > HIST_STALE_HOURS:
        problems.append(('ALERTE', f"hist_curves_meta.json pas régénéré depuis {age_h:.0f}h "
                                   f"(seuil {HIST_STALE_HOURS}h) -- fetch_book_curves.py a peut-être "
                                   f"cessé de PUSHER avec succès (vérifier les logs du workflow "
                                   f"'Fetch Book Curves' : un commit local qui réussit mais dont le "
                                   f"push échoue ne lève PAS d'erreur visible dans le résumé du run)."))
    else:
        total = meta.get('total', {})
        problems.append(('INFO', f"hist_curves_meta.json à jour ({age_h:.0f}h) -- dernier run : "
                                 f"{total.get('match','?')} match / {total.get('set1','?')} set1 / "
                                 f"{total.get('set2','?')} set2 traités."))


def check_hist_partition_sizes(problems):
    """Alerte de manière PROACTIVE avant qu'une partition mensuelle
    (parts/hist_<market>_<YYYY-MM>.jsonl) n'approche la limite dure de
    GitHub (100 Mo) -- exactement le problème qui a bloqué book_curves.jsonl
    5 jours durant. Un mois particulièrement chargé (Grand Chelem + beaucoup
    de tournois simultanés) pourrait en théorie refaire grossir une partition
    jusqu'au mur ; ce check permet d'affiner le découpage (ex. par semaine)
    AVANT que ça arrive, plutôt qu'après."""
    import glob
    # .jsonl ET .jsonl.gz (compression du 16/08/2026)
    parts = glob.glob('parts/hist_*.jsonl') + glob.glob('parts/hist_*.jsonl.gz')
    if not parts:
        return   # rien à vérifier si la migration n'a pas encore eu lieu
    big = []
    for p in parts:
        try:
            ...
            continue
        if mb >= HIST_PARTITION_WARN_MB:
            big.append((p, mb))
    if big:
        detail = ", ".join(f"{p} ({mb:.0f} Mo)" for p, mb in sorted(big, key=lambda x: -x[1]))
        problems.append(('ALERTE', f"Partition(s) proche(s) de la limite GitHub (100 Mo) : {detail} "
                                   f"-- envisager un découpage plus fin (ex. par semaine) avant "
                                   f"que ça bloque le push comme le 14/08/2026."))
...
def check_new_pipeline(problems):
    ...
    if True:
        try:
            ...
            for line in ov.open_curves(CURVES_FILE, verbose=False):
                line = line.strip()
                if not line:
                    continue
                r = json.loads(line)
...
def run_audit():
    ...
    check_tournaments_freshness(problems)
    check_hist_curves_freshness(problems)
    check_hist_partition_sizes(problems)
