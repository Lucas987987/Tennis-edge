...
FOCUS   = [b.strip() for b in os.environ.get('FOCUS', '').split(',') if b.strip()]
    ...
    for src in CURVES:
        ...
        for line in open(src, encoding='utf-8'):
            ...
            if not ct or ct < cut or ct > now:
                continue
            def last_pre(seq):
                pts = [(_dt(t), o) for t, o in (seq or []) if _dt(t) and o and o > 1]
...
def render(rows, label, ndays, ngames):
    ...
    L.append("Marge moyenne (plus bas = meilleur prix rendu) :")
    for r in rows[:10]:
        tag = " 🇫🇷" if (FOCUS and r['book'] in FOCUS) else ""
        L.append(f"  {r['book']}{tag} : {r['marge']:.1f}%  ({r['n']} matchs)")
    top_best = sorted([r for r in rows if r['best']], key=lambda r: -r['best'])[:5]
    if top_best:
        L.append("\nMeilleure cote du panel (fréquence) :")
        for r in top_best:
            L.append(f"  {r['book']} : {r['best']:.0f}% des cas")
    gen = sorted([r for r in rows if r['ecart'] is not None], key=lambda r: -r['ecart'])[:5]
    if gen:
        L.append("\nÉcart au juste prix estimé (positif = au-dessus) :")
        for r in gen:
            s = f" · suivi {'serré' if (r['stab'] or 9) < 3 else 'plus lâche'}" if r['stab'] is not None else ""
            L.append(f"  {r['book']} : {r['ecart']:+.1f}%{s}")
    L.append("\nMéthode : dernier prix pré-match de chaque opérateur, juste prix "
             "par dévig Shin sur la référence Pinnacle. Données horodatées, calcul rejouable.")
    return "\n".join(L) + FOOTER
