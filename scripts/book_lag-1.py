...
EV_TH = 0.02
BUCKETS = [('<30min', 0, 30), ('30-120min', 30, 120), ('2-6h', 120, 360), ('>6h', 360, 1e9)]
...
def main():
    ...
    skipped_disagree = 0
    for uid in common:
        pf, pd = fav_dog(parse_curve(pin[uid].get('home_curve')), parse_curve(pin[uid].get('away_curve')))
        uf, ud = fav_dog(parse_curve(uni[uid].get('home_curve')), parse_curve(uni[uid].get('away_curve')))
        ...
        try:
            ...
            start = uf[-1][0]
        # proba JUSTE de cloture Pinnacle (favori)
    ...
    print()
    print(f">>> Matchs avec fenêtre TARDIVE (<120min, la seule exploitable) : {late_matches}/{n}")
    print()
    print("--- detail par match (meilleure EV vs clôture) ---")
    for m in sorted(matches, key=lambda x: x['best_ev_pct'], reverse=True)[:10]:
        tag = "⏰ TARDIF" if m['fenetre_tardive'] else "(ouverture)"
        print(f"  {m['match'][:32]:32s} EV {m['best_ev_pct']:+6.1f}% à {m['best_mins_avant']:5.0f}min {tag}")
    print("="*66)
    if n < 30:
        print(f"VERDICT : {n} matchs, trop peu. Accumule.")
    elif late_matches / n < 0.15:
        print("VERDICT : la value est surtout à l'OUVERTURE (jeu d'être tôt), pas en retard tardif. "
              "Unibet se cale sur Pinnacle avant le match -> pas de lag tardif exploitable -> ne paie pas le temps réel.")
    else:
        print("VERDICT : des fenêtres TARDIVES existent souvent. Regarde leur ampleur et leur timing : "
              "si tu peux miser à la main dans la fenêtre, le temps réel peut se justifier.")
    return 0
