...
MIN_N_CLUSTER = int(os.environ.get('MIN_N_CLUSTER', '15'))
CLUSTER_RHO   = float(os.environ.get('CLUSTER_RHO', '0.93'))
# TENSION CONNUE entre fraîcheur et robustesse : la fenêtre glissante corrige
# le problème de staleness (voir RECENCY_DAYS ci-dessus), mais réduit le n
# disponible par paire de books -- avec 16 books (120 paires possibles) testées
# simultanément sans correction de tests multiples, un seuil trop permissif
# génère de faux "jumeaux" par pur hasard (constaté : 31 paires à rho>=0.90/
# n>=12, contre 4-18 sur tout l'historique). Seuil relevé à 0.93 et n>=15 en
# compromis ; à democter avec les semaines qui passent (plus de matchs captés
# = plus de puissance statistique à fenêtre de temps égale).
RECENCY_DAYS  = int(os.environ.get('RECENCY_DAYS', '45'))
# FENÊTRE GLISSANTE, pas tout l'historique -- test de stabilité temporelle
# (split-half sur 1046 matchs) : winamax.fr passe de 11,0% de retard moyen
# sur la 1ère moitié chronologique à 0,3% sur la 2e (idem 888sport, bet365).
# Corrélation de rang période1/période2 : rho=+0,31, SOUS le seuil de
# significativité -- le score sur tout l'historique serait dominé par un
# comportement potentiellement obsolète. On calcule donc sur les RECENCY_DAYS
# derniers jours uniquement, pour refléter le comportement actuel.
...
def load_games():
    games = {}
    cutoff = datetime.datetime.utcnow() - datetime.timedelta(days=RECENCY_DAYS)
    # book_curves.jsonl (monolithique) a ete retire le 14/08/2026 (140,73 Mo ->
    # GitHub rejetait tout push > 100 Mo) et remplace par des partitions
    # mensuelles -- voir iter_hist_lines() dans oddspapi_v5.py et
    # migrate_hist_partitions.py pour le contexte complet.
    for line in ov.iter_hist_lines('book'):
        ...
        if not ct or ct < cutoff:
            continue    # hors fenêtre récente -- voir note RECENCY_DAYS ci-dessus
        def pre(seq):
...
def main():
    games = load_games()
    print(f"{len(games)} matchs chargés (live + partitions historiques, {RECENCY_DAYS}j glissants)")
