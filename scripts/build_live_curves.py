...
def main():
    ...
    did = False
    # MARCHE MATCH : depuis matches_oddspapi.json (liste a venir la plus complete).
    # Repli sur live_odds.json (books_match) s'il est absent.
    if os.path.exists(LIVE_IN):
        snap = json.load(open(LIVE_IN, encoding='utf-8'))
        if isinstance(snap, dict):
            snap = list(snap.values())
        print(f"match <- {LIVE_IN} ({len(snap)} matchs)")
        accumulate(OUT_MATCH, rows_from_oddsapi(snap), now, now_iso); did = True
    elif os.path.exists(LIVE_ODDS):
        snap = json.load(open(LIVE_ODDS, encoding='utf-8'))
        if isinstance(snap, dict):
            snap = list(snap.values())
        accumulate(OUT_MATCH, rows_from_books(snap, 'books_match'), now, now_iso); did = True
    # SURFACES SET : depuis live_odds.json (seul a porter les cotes set par book).
    if os.path.exists(LIVE_ODDS):
        snap = json.load(open(LIVE_ODDS, encoding='utf-8'))
        if isinstance(snap, dict):
            snap = list(snap.values())
        print(f"set1/set2 <- {LIVE_ODDS} ({len(snap)} matchs)")
        accumulate(OUT_SET1, rows_from_books(snap, 'books_set1'), now, now_iso)
        accumulate(OUT_SET2, rows_from_books(snap, 'books_set2'), now, now_iso); did = True
    if not did:
        print(f"{LIVE_IN} et {LIVE_ODDS} absents — capture pas encore passee ?")
