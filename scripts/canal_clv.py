...
def _canon(idx, uid):
    """Identifiant canonique du match. DEUX conventions d'uid ont coexisté du
    11/06 au 09/08/2026 (par tournoi puis par date) : 162 matchs existent sous
    deux uid et étaient donc comptés DEUX FOIS. Voir match_key.py."""
    return str(idx.key_of(uid))
...
def main():
    ...
    for line in open(LOG, encoding='utf-8'):
        ...
        for entry in (m.get('retards') or []):
            ...
            k = (date, uid, book)
            if k in ledger:
                # seule exception au gel : compléter la clé canonique si elle
                # manque (ligne écrite avant l'introduction de match_key.py).
    ...
    # Déduplication sur la clé CANONIQUE (match_key.py) et non sur l'uid :
    # deux conventions d'uid ont coexisté du 11/06 au 09/08/2026, 162 matchs
    # existent sous deux uid et étaient comptés deux fois.
    par_match = collections.defaultdict(list)
    for r in retenus:
        par_match[r.get('match_id') or r['uid'] or r['joueur']].append(r['clv'])
