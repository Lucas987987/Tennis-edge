...
def fetch_odds():
    ...
    for fx in fixtures:
        ...
        if psH is None or psA is None:
            print(f"  ⚠️ Pinnacle non extrait pour {name1} vs {name2} (fixture {fx.get('fixtureId')})")
        matches.append({
            'home_team': name1,
            'away_team': name2,
            'commence_time': ct,
...
def main():
    ...
    for m in matches:
        ...
        print(f"  🔎 {home} vs {away} | uid={uid[:40]} | mins_until={round(mins_until)} | psH={psH} psA={psA}")
        if not (psH and psA):
            print(f"     ⏭️ sauté (cotes manquantes)")
            continue
        seen_now.add(uid)
