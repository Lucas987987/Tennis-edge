...
import fr_books            # résolution dynamique des books pariables FR (garde-fou)
...
# CAPTURE_BOOKS=auto  -> liste dynamique des books pariables en France (drapeau API),
#   pinnacle toujours inclus comme "fair" de référence. 1 appel /bookmakers par run.
# CAPTURE_BOOKS=pin,uni,...  -> valeur explicite telle quelle (comportement historique).
# Repli automatique sur fr_books.FALLBACK si le catalogue est indisponible.
CAPTURE_BOOKS = fr_books.resolve(os.environ.get('CAPTURE_BOOKS', fr_books.FALLBACK))
