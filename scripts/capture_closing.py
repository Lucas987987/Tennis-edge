...
def fetch_odds():
    ...
    # IMPORTANT : odds-by-tournaments exige EXACTEMENT un bookmaker (palier gratuit).
    # On prend Pinnacle (référence sharp, la plus utile pour CLV + évaluation).
    # Max/Moyenne multi-bookmakers nécessiteraient 1 requête par bookmaker = trop coûteux
    # pour le quota (250/mois). À reconsidérer si passage à un palier supérieur.
    ...
    for fx in fixtures:
        ...
        books = all_bookmakers_h2h(fx)   # liste {key, home, away} de tous les bookmakers
        matches.append({
            'id': fx.get('fixtureId', ''),
            'home_team': name1,
            'away_team': name2,
