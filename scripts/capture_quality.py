...
def curve_close(curve_raw, start):
    """Dernier prix PRE-match d'une courbe brute [(iso, prix)]."""
...
def main():
    ...
    for uid, e in started:
        ...
        A = curve_close((ce or {}).get('home_curve'), start) if ce else None
        C = curve_close((be or {}).get('home_curve'), start) if be else None
