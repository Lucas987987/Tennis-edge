...
# Niveaux ITF (W15..W125, M15/M25) exclus par defaut pour economiser le quota
# (le backtest ne suit que le circuit + Challengers). INCLUDE_ITF=1 pour reactiver.
INCLUDE_ITF = os.environ.get('INCLUDE_ITF', '0') == '1'

def is_itf_lower(name):
    n = (name or '').strip()
    return re.match(r'^[WMwm][0-9]', n) is not None or 'itf' in n.lower()
    ...
    for tour in ('atp', 'wta'):
        ...
        while True:
            ...
            for fx in items:
                ...
                if tid and name:
                    if not INCLUDE_ITF and is_itf_lower(name):
                        continue   # ITF W/M-series non suivi -> on economise le quota
