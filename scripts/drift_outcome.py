...
# Validation = Sackmann confirmé seulement. Provisoire (api_fast) = directionnel,
# inclus uniquement si INCLUDE_PROVISIONAL=1 (ne valide jamais le CLV).
INCLUDE_PROVISIONAL = os.environ.get('INCLUDE_PROVISIONAL', '0') == '1'
# Closing fiable si dernier point pré-match ≤ ce délai avant le coup d'envoi.
...
def load_results():
    """Liste de (home, away, winner, provisoire). Sackmann (confirmé) prioritaire ;
    le provisoire (api_fast) n'est ajouté que si INCLUDE_PROVISIONAL=1, jamais
    par-dessus un résultat déjà confirmé."""
    out, seen = [], set()
    def key(h, a):
        return frozenset((h.lower().strip(), a.lower().strip()))
    sources = [(RESULTS_FALLBACK, False)]          # resultats.json = confirmé
    if INCLUDE_PROVISIONAL:
        sources.append((RESULTS_FILE, True))       # resultats_fast.json = provisoire
    for path, prov in sources:
        ...
        for r in data.get('results', []):
            ...
            if h and a and w:
                ...
                if k in seen:
                    continue
                seen.add(k); out.append((h, a, w, prov)); added += 1
        print(f"✅ {'PROVISOIRE' if prov else 'confirmé'} {path} : +{added}")
    return out


def fav_won(fav, opp, results):
    for h, a, w, prov in results:
        if (same(fav, h) and same(opp, a)) or (same(fav, a) and same(opp, h)):
            return (1 if same(w, fav) else 0), prov
    return None
...
def main():
    ...
    for line in ov.iter_hist_lines('book'):
        ...
        H = [z for z in sorted((parse(pt[0]), pt[1]) for pt in (x.get('home_curve') or []) if pt[1] and parse(pt[0])) if z[0] <= com]
        A = [z for z in sorted((parse(pt[0]), pt[1]) for pt in (x.get('away_curve') or []) if pt[1] and parse(pt[0])) if z[0] <= com]
        if len(H) < 2 or len(A) < 2:
            continue
        # Récence du closing : dernier point pré-match -> coup d'envoi. Trop ancien = exclu.
        cmb = (com - max(H[-1][0], A[-1][0])).total_seconds() / 60 if com else None
        if cmb is None or cmb > CLV_CLOSE_MAX_MINS:
            n_stale += 1
            continue
        ...
        res = fav_won(fav, opp, results)
        if res is None:
            continue
        won, prov = res
        rows.append({'fav': fav, 'p_open': p_open, 'p_close': p_close,
                     'drift': p_close - p_open, 'won': won, 'prov': prov})

    out = ["## Calibration drift -> résultat (favori, bwin dévigé)", ""]
    if INCLUDE_PROVISIONAL:
        out.append("⚠️ PROVISOIRE INCLUS (INCLUDE_PROVISIONAL=1) — directionnel, NE valide rien.")
    out.append(f"matchs avec drift ET résultat : {len(rows)}")
    out.append(f"closings périmés exclus (>{CLV_CLOSE_MAX_MINS}min) : {n_stale}")
    if not rows:
        print("\n".join(out)); return
