...
BOOKS = [b.strip() for b in os.environ.get('BOOKS', os.environ.get('BOOK', 'unibet')).split(',') if b.strip()]
...
def main():
    ...
    with open(OUT, 'a', encoding='utf-8') as f:
        for book in BOOKS:
            ...
            print(f"[{book}] {len(todo)} matchs à traiter")
            got = 0; warned = False
            for uid, m, fid, start in todo:
                ...
                if not c0 or not c1:
                    # Courbe vide = ce book ne couvre pas ce match. On écrit un stub pour le
                    # marquer 'traité' (il entre dans `done`) et ne plus gaspiller de slot à le ré-essayer.
            ...
            total += got
    print(f"\n✅ {total} courbes récupérées au total ({', '.join(BOOKS)}) — gratuit hors quota")
