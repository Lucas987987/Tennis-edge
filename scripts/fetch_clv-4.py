        ...
        for uid, m, fid, start in todo:
            ...
            if not ch and not ca:
                ...
                if not warned_http:
                    ...
                    warned_http = True
                continue
            # ── GARDE-FOU ORIENTATION (bug v4 du 12/06) ──────────────────
            # L'ordre home/away de l'entree (herite parfois de The Odds API)
            # peut differer du participant1/2 OddsPapi. On verifie contre
            # l'HISTORY de la capture (meme source que les noms) : si les
            # courbes collent mieux en INVERSE, on les re-swappe avant analyse.
            hist_ref = [p for p in (m.get('history') or [])
                        if p.get('mins_before') is not None and p['mins_before'] >= 0
                        and p.get('home') and p.get('away')]
            if hist_ref and ch and ca:
                cap_h, cap_a = float(hist_ref[-1]['home']), float(hist_ref[-1]['away'])
                pre_h = [pr for dt, pr in ch if dt <= start] if start else [pr for _, pr in ch]
                pre_a = [pr for dt, pr in ca if dt <= start] if start else [pr for _, pr in ca]
                if pre_h and pre_a:
                    d_dir = abs(pre_h[-1] - cap_h) / cap_h + abs(pre_a[-1] - cap_a) / cap_a
                    d_swp = abs(pre_a[-1] - cap_h) / cap_h + abs(pre_h[-1] - cap_a) / cap_a
                    if d_swp < 0.5 * d_dir:
                        ch, ca = ca, ch
                        print(f"  🔁 orientation corrigee (ordre v4 ≠ OddsPapi): {m.get('home')} vs {m.get('away')}")
