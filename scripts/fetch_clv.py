...
# Récence max du closing rétroactif : un dernier point capté plus de
# CLOSING_MAX_MINS avant le coup d'envoi n'est PAS un vrai closing (courbe OddsPapi
# arrêtée trop tôt). Réglable ; 35 = strict (= snapshots), 60 = tolérant.
CLOSING_MAX_MINS = int(os.environ.get('CLOSING_MAX_MINS', '60'))
...
def analyze_side(curve, start):
    ...
    close_t = None
    if start:
        before = [(dt, pr) for dt, pr in curve if dt <= start]
        if before:
            close_t, close_price = before[-1]
        else:
    ...
    # Récence du closing : combien de minutes AVANT le coup d'envoi date le dernier
    # point retenu. Sert à exclure les courbes arrêtées trop tôt (faux closings).
    cmb = round((start - close_t).total_seconds() / 60) if (start and close_t) else None
    return {'open':open_price,'close':close_price,'move_pct':move_pct,'amplitude_pct':amp,
            ...
            'close_t': close_t.isoformat() if close_t else None,
            'close_mins_before': cmb,
            'close_reliable': (cmb is not None and cmb <= CLOSING_MAX_MINS)}
    ...
    with open(CLV_LOG, 'a', encoding='utf-8') as logf:
        for uid, m, fid, start in todo:
            ...
            if not ok or not hist:
                ...
                continue
            try:
                c0, c1 = parse_two_curves(hist)
                ch, ca = c0, c1   # outcome0 -> home par defaut
                rh, ra = ref_home_away(m)
                if rh and c0 and c1 and abs(c0[-1][1]-rh) > abs(c1[-1][1]-rh):
                    ch, ca = c1, c0   # swap : c1 colle mieux au prix home connu
            except Exception as e:
