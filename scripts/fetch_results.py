...
def enrich_backtest(D):
    ...
    for line in lines[1:]:
        ...
        if pA:
            set_col(iEloA, pA.get('elo',{}).get(ek) or pA.get('elo',{}).get('elo'))
            ...
            form_a = pA.get('form',{}).get(sk)
            if form_a:
                set_col(iFsA, f"{form_a.get('w',0)}V{form_a.get('l',0)}D")

        if pB:
            set_col(iEloB, pB.get('elo',{}).get(ek) or pB.get('elo',{}).get('elo'))
