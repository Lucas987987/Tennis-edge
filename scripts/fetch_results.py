...
def get_recent_matches(csv_content, days=7):
    ...
    # Debug : afficher la dernière date disponible
    if rows:
        dates = [r.get('tourney_date','0') for r in rows if r.get('tourney_date','0').isdigit()]
        if dates:
            last_date = max(dates)
            print(f"  Dernière date dans CSV: {last_date}")
            print(f"  Cutoff cherché: {cutoff_int}")
...
def main():
    ...
    for repo, filename, circuit in datasets:
        ...
        try:
            ...
            # Chercher sur 30 jours pour trouver ce qui est disponible
            matches = get_recent_matches(csv_content, days=30)
            print(f"  {len(matches)} matchs sur 30 jours trouvés")
    ...
    if results:
        print(f"\nExemples trouvés:")
        for r in results[:3]:
            print(f"  {r['date']} - {r['home_team']} bt {r['away_team']} ({r['tournament']})")
