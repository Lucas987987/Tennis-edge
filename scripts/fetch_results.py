...
def enrich_backtest(D):
    """Met à jour le CSV backtest avec Elo/stats et résultats."""
    ...
    iEloA = headers.index('elo_a') if 'elo_a' in headers else -1
    iEloB = headers.index('elo_b') if 'elo_b' in headers else -1
    iSrvA = headers.index('srv_a') if 'srv_a' in headers else -1
    iSrvB = headers.index('srv_b') if 'srv_b' in headers else -1
    iRetA = headers.index('ret_a') if 'ret_a' in headers else -1
    iRetB = headers.index('ret_b') if 'ret_b' in headers else -1
    iJA   = headers.index('joueurA') if 'joueurA' in headers else -1
    iJB   = headers.index('joueurB') if 'joueurB' in headers else -1
    iSurf = headers.index('surf') if 'surf' in headers else -1
    ...
    for line in lines[1:]:
        if not line.strip():
            ...
            continue
        cols = line.split(';')
        # Enrichir seulement si elo_a vide
        if iEloA >= 0 and iJA >= 0 and (not cols[iEloA] or cols[iEloA] == ''):
            name_a = cols[iJA] if iJA < len(cols) else ''
            name_b = cols[iJB] if iJB >= 0 and iJB < len(cols) else ''
            surf = cols[iSurf] if iSurf >= 0 and iSurf < len(cols) else 'Hard'
            ek = elo_key(surf)
            pA = find_player(D, name_a)
            pB = find_player(D, name_b)
            if pA and iEloA >= 0:
                cols[iEloA] = str(pA.get('elo', {}).get(ek) or pA.get('elo', {}).get('elo') or '')
            if pA and iSrvA >= 0:
                cols[iSrvA] = str(pA.get('serve', {}).get('spw') or '')
            if pA and iRetA >= 0:
                cols[iRetA] = str(pA.get('ret', {}).get('rpw') or '')
            if pB and iEloB >= 0:
                cols[iEloB] = str(pB.get('elo', {}).get(ek) or pB.get('elo', {}).get('elo') or '')
            if pB and iSrvB >= 0:
                cols[iSrvB] = str(pB.get('serve', {}).get('spw') or '')
            if pB and iRetB >= 0:
                cols[iRetB] = str(pB.get('ret', {}).get('rpw') or '')
            if pA or pB:
                updated += 1
