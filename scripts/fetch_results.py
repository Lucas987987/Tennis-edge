...
def enrich_closing_lines():
    ...
    for line in lines[1:]:
        ...
        for uid, cl in closing.items():
            ...
            # Cote de référence AUTOMATIQUE = cote d'ouverture (1er point de l'historique).
            # Indépendante du moment de chargement par l'utilisateur.
            hist = cl.get('history', [])
            ref_a = None
            if hist:
                first = hist[0]
                ref_a = first['home'] if a_is_home else first['away']
            # Fallback : si pas d'historique, utiliser la coteA de chargement
            if not ref_a or ref_a <= 1:
                try:
                    ref_a = float(coteA) if coteA else 0
                except: ref_a = 0
            if not ref_a or ref_a <= 1: break

            # CLV 60min : cote ouverture vs Pinnacle 60min (recalcul systématique)
            # Champ ponctuel si présent, sinon reconstruit depuis l'history (~60min avant).
            ...
            # CLV 15min : cote ouverture vs Pinnacle 15min (recalcul systématique)
            # Champ ponctuel si présent, sinon dernier prix avant match (~15min) via history.
