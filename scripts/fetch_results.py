...
def enrich_backtest(D):
    ...
    print(f"  [diag] index: rank_a={iRkA}, rank_b={iRkB}")
    # Compter combien de joueurs du backtest ont un rank
    _diag_rank, _diag_norank, _diag_notfound = 0, 0, 0
    for _l in lines[1:]:
        if not _l.strip(): continue
        _c = _l.split(';')
        _ja = _c[iJA] if iJA < len(_c) else ''
        _p = find_player(D, _ja)
        if not _p: _diag_notfound += 1
        elif _p.get('rank') is not None: _diag_rank += 1
        else: _diag_norank += 1
    print(f"  [diag] joueurs A: {_diag_rank} avec rank, {_diag_norank} sans rank, {_diag_notfound} non trouvés")
    # Vérifier si rank_a est DÉJÀ rempli dans le CSV
    _already = 0
    for _l in lines[1:]:
        if not _l.strip(): continue
        _c = _l.split(';')
        if iRkA < len(_c) and _c[iRkA] not in ('', 'nan'):
            _already += 1
    print(f"  [diag] rank_a déjà rempli dans CSV: {_already}/{len(lines)-1}")
