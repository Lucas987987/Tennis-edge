...
def tour_tokens(s):
    """Tokens significatifs d'un nom de tournoi (sans ATP/WTA/Open/etc.) pour
    rapprocher 'ATP Stuttgart' et 'ATP Boss Open - Stuttgart'."""
    stop = {'atp','wta','itf','open','cup','championships','championship','masters',
            'challenger','men','women','mens','womens','singles','doubles','the','de',
            'tour','trophy','classic','international','grand','slam','presented','by','of'}
    return frozenset(w for w in norm_name(s).split(' ') if len(w) >= 3 and w not in stop)
...
def run():
            ...
            if min(iJA, iJB, iRes, iTour) < 0:
                ...
                for r in rows:
                    while len(r) < len(headers):
                        r.append('')
                if iSrc < 0:                       # créer src_resultat si absente
                    headers.append('src_resultat')
                    for r in rows:
                        r.append('')
                    iSrc = len(headers) - 1
                    needs_src_col = True
    ...
    all_parsed = []            # (nm1,nm2,vainqueur,score,tournoi) tous tournois
    # Économie de quota : n'interroger que les tournois ayant un signal NON résolu
    relevant_tokens = set()
    if rows is not None and iTour >= 0 and iRes >= 0:
        for r in rows:
            cell = r[iRes] if iRes < len(r) else ''
            if cell.strip() in ('', 'nan'):
                relevant_tokens |= tour_tokens(r[iTour] if iTour < len(r) else '')
    if relevant_tokens:
        print(f"⚡ Quota : {len(relevant_tokens)} tokens en attente -> on saute les tournois sans signal")
    ...
    for tname, info in tourn_ids.items():
        if tname.startswith('_') or not isinstance(info, dict):
            continue
        if relevant_tokens and not (tour_tokens(tname) & relevant_tokens):
            continue   # aucun signal backtest en attente -> requête économisée
        ...
        all_parsed += [(a, b, w, res, tname) for (a, b, w, res) in api_parsed]

        # (enrichissement CSV déplacé après la boucle : matching par joueurs)

    # ── Enrichissement CSV par JOUEURS (sans exiger l'égalité du nom de tournoi) ──
    if rows is not None:
        updated = 0; ambigus = 0
        for r in rows:
            if r[iRes].strip() not in ('', 'nan'):
                continue
            ja, jb = r[iJA], r[iJB]
            cands = [(w, res, tn) for (a, b, w, res, tn) in all_parsed
                     if (same_player(ja, a) and same_player(jb, b))
                     or (same_player(ja, b) and same_player(jb, a))]
            if not cands:
                continue
            # plusieurs résultats contradictoires sur le vainqueur ? tenter de
            # désambiguïser par recouvrement du nom de tournoi, sinon ignorer.
            if len({same_player(c[0], ja) for c in cands}) > 1:
                tb = tour_tokens(r[iTour])
                narrowed = [c for c in cands if tour_tokens(c[2]) & tb]
                if narrowed and len({same_player(c[0], ja) for c in narrowed}) == 1:
                    cands = narrowed
                else:
                    ambigus += 1; continue
            win_name, result_str, _tn = cands[0]
    ...
    if (csv_changed or needs_src_col) and headers is not None:
