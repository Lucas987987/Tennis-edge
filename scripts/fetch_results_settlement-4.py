...
def main():
    ...
    for fid, m in matches.items():
        ...
        if prev and (prev.get('winner') or prev.get('void')):
            # Backfill set1/set2 : entree resolue avant l'ajout des champs. On
            # re-interroge UNE fois puis on marque (False = indispo definitif).
            # RETRY_FALSE=1 (env) : re-tente AUSSI les entrees marquees False —
            # a utiliser apres correction d'un bug d'extraction.
            retry_false = os.environ.get('RETRY_FALSE') == '1'
            missing = ('set1' not in prev or 'set2' not in prev)
            frozen = (prev.get('set1') is False or prev.get('set2') is False)
            if prev.get('winner') and (missing or (retry_false and frozen)):
    ...
    for fid, m in backfill:
        ...
        # Ne jamais DEGRADER une valeur deja connue ('home'/'away' reste) ;
        # False/absent peut etre rempli ou re-marque False.
        if store[fid].get('set1') not in ('home', 'away'):
            store[fid]['set1'] = r.get('set1') if r.get('set1') else False
        if store[fid].get('set2') not in ('home', 'away'):
