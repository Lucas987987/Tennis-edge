...
def _parse_book_markets(bkm, home, away):
    """Extrait {'spreads':..., 'totals':...} d'UN bookmaker (ou None si absent)."""
    out = {'spreads': None, 'totals': None}
    for mkt in bkm.get('markets', []):
        key = mkt.get('key')
        outs = mkt.get('outcomes', [])
        if key == 'spreads' and out['spreads'] is None:
            sp = {}
            for o in outs:
            ...
            if sp:
                out['spreads'] = sp
        elif key == 'totals' and out['totals'] is None:
            to = {}
            for o in outs:
...
def _extract_all_books_markets(match):
    """Extrait spreads+totals de TOUS les books présents dans le payload.
    Aucun appel API supplémentaire : les books sont déjà dans la réponse (même
    région, même requête). On parse simplement ce qu'on jetait avant.
    Retourne {book_key: {'spreads':..., 'totals':...}} (books sans marché exclus)."""
    home = match.get('home_team', '')
    away = match.get('away_team', '')
    books = {}
    for bkm in match.get('bookmakers', []):
        key = (bkm.get('key') or '').lower()
        if not key:
            continue
        mk = _parse_book_markets(bkm, home, away)
        if mk['spreads'] or mk['totals']:
            books[key] = mk
    return books


def _extract_pinnacle_markets(match):
    """Rétro-compat : renvoie le marché Pinnacle (ou 1er book dispo) au format d'origine.
    Réimplémenté au-dessus de _extract_all_books_markets (même résultat, 0 requête)."""
    books = _extract_all_books_markets(match)
    if not books:
        return {'spreads': None, 'totals': None, 'book': None}
    # priorité à pinnacle, sinon 1er book disponible
    chosen = next((b for b in books if 'pinnacle' in b), next(iter(books)))
    return {'spreads': books[chosen]['spreads'], 'totals': books[chosen]['totals'], 'book': chosen}
...
def run_games_collector(api_key):
    ...
    for sk in sport_keys_needed:
        ...
        for match in data:
            ...
            mk = _extract_pinnacle_markets(match)          # rétro-compat (champ Pinnacle)
            all_books = _extract_all_books_markets(match)  # NOUVEAU : tous les books
            ...
            point = {
                ...
                'spreads': mk['spreads'],          # Pinnacle (rétro-compat)
                'totals': mk['totals'],            # Pinnacle (rétro-compat)
                'book': mk['book'],
                'books': all_books,                # NOUVEAU : {book: {spreads, totals}} tous books
                'available': bool(all_books),
            }
