...
def bloc_signaux(chemin='h13_clv_detail.csv'):
    """CLV et ROI des alertes RÉELLEMENT ENVOYÉES par h13_signal.py.

    À ne pas confondre avec le bloc principal, qui porte sur la population
    de l'hypothèse gelée (moves_detail_hist.csv, reconstruite en batch).
    Les deux devraient converger ; un écart durable signalerait que la
    détection temps réel a dérivé de la reconstruction — le seul défaut
    capable de rendre ce dispositif silencieusement trompeur.

    Produit par h13_clv.py, qui tourne juste avant dans steam_pipeline.
    """
    try:
        lignes = list(csv.DictReader(open(chemin, encoding='utf-8')))
    except OSError:
        return None
    clv, pnl, wins, n_res, deduits = [], [], 0, 0, 0
    for r in lignes:
        if r.get('side_deduit') == 'oui':
            deduits += 1
        try:
            if r.get('clv_book_pct'):
                clv.append(float(r['clv_book_pct']))
        except ValueError:
            pass
        if r.get('gagne') in ('oui', 'non'):
            n_res += 1
            wins += 1 if r['gagne'] == 'oui' else 0
            try:
    ...
    if not lignes:
        return None
    return {'n': len(lignes), 'clv': clv, 'pnl': pnl, 'wins': wins,
            'n_res': n_res, 'deduits': deduits}
...
def message(outs, ins, hier):
    ...
    sig = bloc_signaux()
    if sig and sig['clv']:
        k = sum(1 for v in sig['clv'] if v > 0)
        L.append(f"\n<b>Alertes réellement envoyées</b> : {sig['n']} signal(aux)")
        L.append(f"CLV médian {st.median(sig['clv']):+.1f} % · "
                 f"positif {k}/{len(sig['clv'])} = "
                 f"{100 * k / len(sig['clv']):.0f} %")
        if sig['pnl']:
            L.append(f"ROI {100 * st.mean(sig['pnl']):+.1f} % sur "
                     f"{sig['n_res']} dénoué(s) — <i>indicatif, le CLV se "
                     f"stabilise bien plus vite</i>")
        if sig['deduits']:
