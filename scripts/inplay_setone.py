...
def main():
    ...
    with open(cpath, encoding='utf-8') as f:
        for line in f:
            ...
            peak, t_peak, n_in = peak_after(fav_curve_pts, commence, PEAK_WIN)
            if peak is None:
                diag['no_inplay'] += 1
                continue
            opp = price_at(opp_curve_pts, t_peak)
            if not opp or opp <= 1 or peak <= 1:
                continue
            implied_live = (1 / peak) / ((1 / peak) + (1 / opp))
            fav_won_match = (res['wc'] == 1 and fav_res == 'home') or (res['wc'] == 2 and fav_res == 'away')
            fav_open = min(ho, ao)
            diag['usable'] += 1
            rows.append({'open': fav_open, 'implied_live': implied_live,
                         'won': 1 if fav_won_match else 0})
    ...
    print("⚠ pic de cote = proxy du post-set-1 ; timing exact = via marchés de set (à venir). "
          "Quantité in-play, descriptive — exécution live = mur connu.")
...
def selftest():
    ...
    for i in range(60):
        ...
        # favori = home, ouverture 1.55 ; pic in-play ~2.4 après perte du set 1 ; opp ~1.55
        hc = [["2026-06-06T17:00:00Z", 1.55], ["2026-06-06T17:55:00Z", 1.54],
              ["2026-06-06T18:25:00Z", 2.1], ["2026-06-06T18:40:00Z", 2.4],
              ["2026-06-06T19:30:00Z", 1.6 if fav_wins else 5.0]]
        ac = [["2026-06-06T17:00:00Z", 2.45], ["2026-06-06T18:40:00Z", 1.55]]
