...
def http_get(path, params=None):
    ...
    req = urllib.request.Request(url, headers={
        ...
        'User-Agent': 'TennisEdge-KalshiProbe/2.0'})
...
def matchs_locaux():
    ...
    for x in d:
        ...
        out.append({'home': h, 'away': a,
                    ...
                    # « Qf1 vs Qf2 » : tours futurs, se rempliront au fil du
                    # tournoi. Comptés à part, jamais jetés en silence.
...
def main():
    ...
    for i, s in enumerate(cibles, 1):
        ...
        try:
            ...
            if erreurs <= 3:
                print(f"   ⚠️ {tk} : {e}")
        time.sleep(0.06)          # ~20 req/s autorisées
    ...
    details = {}
    for loc in reels:
        trouve = [m for m in marches
                  if joueur_match(loc['home'], txt(m))
                  and joueur_match(loc['away'], txt(m))]
        if not trouve:
            continue
        spreads = []
        for m in trouve:
