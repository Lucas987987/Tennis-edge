...
def _as_book_dict(c):
    """Normalise en dict slug->data, que la source soit déjà un dict OU une liste
    d'objets {key/slug/title, ...} (selon la forme renvoyée par /odds)."""
    ...
    if isinstance(c, list):
        ...
        for b in c:
            if isinstance(b, dict):
                k = b.get('key') or b.get('slug') or b.get('title')
                if k:
                    out[k] = b
        return out
    return {}
...
def extract_books(resp, fid):
    """Trouve les books quelle que soit la clé/imbrication ET la forme (dict ou liste)."""
    if not isinstance(resp, dict): return {}
    for key in ('bookmakerOdds', 'bookmakers'):
        if key in resp:
            r = _as_book_dict(resp[key])
            if r: return r
...
def main():
    ...
    print("\n  --- liste complète ---")
    print("   ", ", ".join(slugs))
    # Ligne prête à coller dans le workflow : pinnacle (sharp) en tête + books FR détectés.
    fr_soft = [x for x in fr if x != 'pinnacle']
    suggested = (['pinnacle'] if 'pinnacle' in slugs else []) + fr_soft
    if suggested:
        print("\n  >>> à coller dans le workflow fetch_book_curves (env) :")
        print(f"      BOOKS: {','.join(suggested)}")
        print("      (pinnacle = ta référence sharp ; les autres = books mous misables en France)")
