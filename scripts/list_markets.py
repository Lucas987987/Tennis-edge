...
def catalogue():
    ...
    for m in items:
        ...
        if mid is None:
            continue
        # le libelle peut etre sous des cles variees selon l'API : on prend la
        # premiere valeur texte non vide qui ressemble a un nom
        name = ''
        for k in ('name', 'title', 'market', 'marketName', 'label', 'description',
                  'shortName', 'displayName', 'key', 'slug'):
            v = m.get(k)
            if isinstance(v, str) and v.strip():
                name = v.strip(); break
        if not name:
            # dernier recours : afficher la structure brute pour diagnostic
            name = '{' + ', '.join(f"{k}={repr(v)[:20]}" for k, v in list(m.items())[:5]) + '}'
        out[str(mid)] = name
    print(f"=== /markets : {len(out)} marchés au catalogue ===")
    if items and isinstance(items[0], dict):
        print("  structure d'une entree brute :")
