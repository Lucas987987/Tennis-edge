...
def main():
    ...
    with open(CLV_LOG, encoding='utf-8') as f:
        for line in f:
            ...
            if e.get('home_curve') and e.get('away_curve'): n_with_curve += 1
            r = analyze_match(e)
            if r: recs.append(r)
    ...
    # Cas « rien à analyser » : on sort proprement avec un diagnostic (pas de crash)
    if not recs:
        print(f"📊 0 match analysable.")
        print(f"   Lignes lues dans {CLV_LOG} : {n_lines}")
        print(f"   Entrées avec courbes (home_curve + away_curve) : {n_with_curve}")
        if n_lines == 0:
            print("   → clv_history.jsonl est vide : laisse tourner fetch_clv pour accumuler.")
        elif n_with_curve == 0:
            print("   → aucune courbe stockée (entrées antérieures à STORE_FULL_CURVE=True). Les nouveaux matchs en auront.")
        else:
            print("   → courbes présentes mais trop courtes (<2 points exploitables).")
        with open(OUT_REPORT, 'w', encoding='utf-8') as f:
            json.dump({'n_matchs': 0, 'lignes_lues': n_lines,
                       'entrees_avec_courbe': n_with_curve}, f, ensure_ascii=False, indent=2)
        print(f"\n✅ Rapport: {OUT_REPORT} (vide, en attente de données)")
        return
    ...
    if report['amplitude_moyenne_pct'] is not None:
