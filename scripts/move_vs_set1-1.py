...
# Frontiere out-of-sample : les matchs AVANT cette date ont GENERE l'hypothese
# (gros move -> residu positif) ; seuls ceux d'APRES peuvent la tester.
SPLIT_DATE = os.environ.get('SPLIT_DATE', '2026-06-12')
...
def main():
    ...
    # ── OUT-OF-SAMPLE : matchs >= SPLIT_DATE uniquement ──
    fresh = [r for r in rows if r['ct'] >= SPLIT_DATE]
    print(f"\n── OUT-OF-SAMPLE (matchs >= {SPLIT_DATE}) — {len(fresh)} matchs ──")
    print("(seul terrain de test legitime de l'hypothese 'gros move' ; tout le reste est in-sample)")
    if fresh:
        groups_f = {}
        for r in fresh:
            groups_f.setdefault(bucket_of(r['steam_move']), []).append(r)
        for lo, hi, label in BUCKETS:
            g = groups_f.get(label, [])
            if not g:
                continue
            n = len(g)
            pc = sum(r['p_close'] for r in g) / n
            ...
            ws1 = (sum(r['won_set1'] for r in s1) / len(s1)) if s1 else None
            tag = '' if n >= MIN_N else '  [DIRECTIONNEL n<%d]' % MIN_N
            ws1_s = f"{ws1:.1%}" if ws1 is not None else '-'
            print(f"  {label:<20} n={n:<3} match {wm:.1%} vs close {pc:.1%} (residu {wm - pc:+.1%})"
                  f" | set1 {ws1_s} (n={len(s1)})" + tag)
