...
MAX_AMP_PCT = float(os.environ.get('MAX_AMP_PCT', '35'))  # garde-fou anti-corruption (voir _compute_move)
...
def _compute_move(uid, m):
    ...
    # GARDE-FOU ANTI-CORRUPTION : un mouvement PRE-MATCH > MAX_AMP_PCT n'est
    # (quasi) jamais reel -- signature connue d'un decalage entre l'heure
    # officielle (commence_time) et le vrai debut du match (le match a
    # commence "en avance" sur le planning), ou d'un point de capture
    # corrompu. Meme protection que celle deja validee sur canal_public.py
    # (cas Hurkacz/Popyrin) : un plafond de sanite independant de toute
    # logique de date/heure, donc robuste meme si mins_before/commence_time
    # sont eux-memes faux. Cas declencheur : Perricard vs Svajda, alerte a
    # "T-9 min" affichant +111.6% alors que le match etait deja en cours.
    if amp > MAX_AMP_PCT:
        return None
...
def x_search_url(home, away):
    ...
    Gratuit, zero API -- meme fonction que dans steam_alert.py/canal_public.py."""
    def last(name):
