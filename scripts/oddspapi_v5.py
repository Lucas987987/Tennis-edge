...
def current_hist_partition_path(market='book'):
    ...
    path = base
    seq = 1
    # Une partition GZIPPÉE est close et ne doit plus jamais être réécrite en
    # clair : on passe au suffixe suivant comme si elle était pleine.
...
def iter_hist_lines(market='book'):
    """Itère sur TOUT l'historique d'un marché (partitions + fichier
    monolithique legacy s'il existe encore), comme un flux unique de lignes
    JSONL brutes (chaque ligne = 1 courbe book/match, format inchangé).

    COMPRESSION (16/08/2026) : les partitions closes sont gzippées
    (parts/hist_<market>_*.jsonl.gz) par compress_hist_partitions.py. Le gain
    mesuré est de 86 % (97,7 Mo -> 14,1 Mo sur hist_book_2026-W33), soit
    435 Mo -> ~63 Mo sur l'ensemble de l'historique. Nécessaire parce que
    parts/ grossissait de ~80 Mo par semaine sans jamais être purgé
    (purge_old_partitions ne filtre que parts/live_*, et HIST_DAYS=0 par choix :
    l'historique EST l'actif de recherche du projet).
    AUCUNE PERTE : gzip est sans perte, et la lecture est transparente ici —
    les deux formes coexistent, aucun script consommateur n'a à changer.
    Si un .jsonl et son .jsonl.gz existent tous les deux (compression en cours),
...
def open_any(path):
    ...
    Depuis la compression du 16/08/2026, une partition historique peut n'exister
...
def hist_partitions(market='book'):
    """Chemins des partitions d'un marché, .jsonl et .jsonl.gz confondus.
    Le .gz est renvoyé SANS son suffixe : open_any() se charge de le résoudre,
    et les appelants gardent des chemins lisibles."""
    plain = glob.glob(f'parts/hist_{market}_*.jsonl')
    gz = [p[:-3] for p in glob.glob(f'parts/hist_{market}_*.jsonl.gz')]
    return sorted(set(plain) | set(gz))
...
def open_curves(path, verbose=True):
    ...
    if base in LEGACY_HIST and not os.path.exists(path):
        ...
        # Compter aussi les partitions COMPRESSÉES (.jsonl.gz) : depuis la
        # compression du 16/08/2026, l'historique peut n'exister que sous
        # cette forme -- un glob sur '.jsonl' seul concluait à tort à l'absence
        # totale d'historique et faisait échouer tous les consommateurs.
        n = (len(glob.glob(f'parts/hist_{market}_*.jsonl'))
             + len(glob.glob(f'parts/hist_{market}_*.jsonl.gz')))
        if verbose:
            print(f"  [open_curves] {base} absent -> {n} partition(s) parts/hist_{market}_*")
        if n == 0:
            raise FileNotFoundError(f"{base} absent ET aucune partition parts/hist_{market}_*")
