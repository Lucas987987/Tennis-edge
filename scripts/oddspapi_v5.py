...
def settlement_result(fixture_id):
    """
    Resout le vainqueur via le settlement du marche vainqueur (121).
    Renvoie {finished, winner('home'/'away'/None), p1_sets, p2_sets, status_name}.
    - winner='home' si l'outcome 121 est 'won' ; 'away' si 122 'won'.
    - secours : deduit du score en sets (scores.result) si pas de settlement 121.
    """
    out = {"finished": False, "winner": None,
    ...
    for s in (data.get("settlements") or []):
        if s.get("marketId") == MW_MARKET and s.get("status") == "won":
            oid = s.get("outcomeId")
            if oid == OUT_HOME:
                out["winner"] = "home"
            elif oid == OUT_AWAY:
