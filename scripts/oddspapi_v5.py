...
def current_hist_partition_path(market='book'):
    ...
    DÉCOUPAGE HEBDOMADAIRE (depuis le 15/08/2026), pas mensuel : la partition
    d'août 2026 a atteint 86 Mo en quelques jours seulement (alerte
    check_hist_partition_sizes, seuil 80 Mo) -- à ce rythme elle aurait
    retouché le mur des 100 Mo de GitHub avant la fin du mois. Une semaine
    de ce même volume pèse ~20-22 Mo, avec une bien meilleure marge de
    sécurité. Format ISO (YYYY-Www) : sans ambiguïté, trie naturellement.
    RÉTROCOMPATIBLE sans rien migrer : iter_hist_lines() fait un glob sur
    'hist_<market>_*.jsonl', qui ramasse aussi bien les anciennes partitions
    MENSUELLES (hist_book_2026-06.jsonl) que les nouvelles HEBDOMADAIRES
    (hist_book_2026-W33.jsonl) -- aucun changement requis côté lecture."""
    wk = datetime.datetime.utcnow().strftime('%G-W%V')
    os.makedirs('parts', exist_ok=True)
    return f'parts/hist_{market}_{wk}.jsonl'
