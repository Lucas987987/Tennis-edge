...
def load_results():
    """Sackmann (officiel) prioritaire ; complété par le provisoire rapide
    UNIQUEMENT pour les matchs que Sackmann n'a pas encore. Comme find_winner_side
    scanne dans l'ordre, l'officiel l'emporte dès qu'il arrive ('Sackmann prend la place')."""
...
def main():
    ...
    for book, rows in by_book.items():
        ...
        soft_found = 0; no_result = 0; n_eval = 0
        for br in rows:
            uid = br.get('uid')
        ...
        n = len(flagged)
        if n == 0:
            report['books'][book] = {'flagged': 0, 'matchs_evalues': n_eval,
