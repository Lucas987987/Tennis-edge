...
JOURNAL = os.environ.get('JOURNAL', 'paper_trades.jsonl')
...
def settle_trade(t, data, results):
    ...
    # settle seulement si on a au moins la cloture
...
def main():
    ...
    softbooks = sorted({b for m in data.values() for b in m
                        if not b.startswith('_') and b != sa.SHARP})
    if sa.SOFT_PREF:
    ...
    if BACKFILL:
        print("MODE BACKFILL : rejoue tout l'historique (ne persiste pas).")
        trades = {}
    ...
    for t in trades.values():
        if t.get('status') == 'OPEN':
            ...
            if bk and bk.get('_commence') and bk['_commence'] < now:
                if settle_trade(t, data, results):
                    n_settled += 1
