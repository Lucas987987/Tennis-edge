...
def ticks_path() -> Path:
    """Partition du jour (UTC). Recalculée à chaque écriture : un collecteur qui
    tourne à cheval sur minuit bascule seul sur la partition suivante."""
    if TICKS_FILE_OVERRIDE:
        return Path(TICKS_FILE_OVERRIDE)
    return TICKS_DIR / f"pm_ticks_{utc_now().strftime('%Y-%m-%d')}.jsonl"
...
def main() -> int:
    ...
    while not STOP.is_set() and (run_deadline is None or time.monotonic() < run_deadline):
        ...
        if run_deadline is not None:
            ws_seconds = min(ws_seconds, max(30, int(run_deadline - time.monotonic())))
