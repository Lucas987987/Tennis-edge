...
def find_commence(fid):
    """Récupère le commence_time du match dans les fichiers locaux du repo."""
    for path in (os.environ.get('BOOK_CURVES', 'book_curves.jsonl'),):
        if glob.glob(path):
            with open(path, encoding='utf-8') as f:
                for line in f:
                    try:
                        ...
                        continue
                    if str(r.get('fixture_id')) == str(fid):
                        c = parse(r.get('commence_time'))
                        if c:
                            return c, path
    if glob.glob('matches_oddspapi.json'):
        try:
            arr = json.load(open('matches_oddspapi.json', encoding='utf-8'))
        except (json.JSONDecodeError, OSError):
            arr = []
        for m in (arr if isinstance(arr, list) else []):
            if str(m.get('id')) == str(fid):
                c = parse(m.get('commence_time'))
                if c:
                    return c, 'matches_oddspapi.json'
    return None, None
...
def main():
    ...
    src = "fourni"
    if commence is None:
        commence, where = find_commence(fid)
        src = f"auto ({where})" if commence else "introuvable"
    print(f"fixtureId={fid} | commence={commence} [{src}] "
          f"{'(décalages calculés)' if commence else '(décalages absents -> donne COMMENCE ou place book_curves.jsonl)'}\n")
