...
def check_secondary_markets():
    ...
    print(f"\n=== {len(upcoming)} matchs à venir aujourd'hui | test sur {len(fids)} ===")
    if not fids:
        print("  Aucun match à venir trouvé -> impossible de tester les marchés secondaires "
              "(réessayer à un autre moment de la journée).")
        return
