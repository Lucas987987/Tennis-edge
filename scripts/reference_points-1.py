...
def main():
    ...
    # Marge a la CLOTURE par SEGMENT (proxy de liquidite/confiance de Pinnacle :
    # une marge plus large = book moins sur de lui = la ou une inefficience
    # residuelle aurait le plus de chances de vivre).
    seg_m = {}
    for x in entries:
        hist = [p for p in (x['e'].get('history') or [])
                if p.get('mins_before') is not None and 0 <= p['mins_before'] <= 60
                and p.get('home') and p.get('away')]
        if not hist:
            continue
        mg = margin(hist[-1]['home'], hist[-1]['away'])
        if mg is None:
            continue
        t = str(x['e'].get('tournament') or '')
        genre = 'WTA' if 'wta' in t.lower() else ('ATP' if 'atp' in t.lower() else 'autre')
        niv = x['e'].get('niveau') or '?'
        seg_m.setdefault(('genre', genre), []).append(mg)
        seg_m.setdefault(('niveau', niv), []).append(mg)
    if seg_m:
        print("  marge de cloture (<=60min) par segment :")
        for (kind, lab), vals in sorted(seg_m.items()):
            print(f"    {kind} {lab:<14} n={len(vals):<4} marge {st.mean(vals):.2%}" + tag(len(vals)))
    if len(closes_m) >= 6:
