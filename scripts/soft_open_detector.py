...
SHARP_BOOK  = os.environ.get('SHARP_BOOK', 'pinnacle')  # vraie réf sharp (était bwin)
...
# Closing sharp jugé fiable si son dernier point pré-match est ≤ ce délai avant le coup
# d'envoi. Au-delà, la courbe s'est arrêtée trop tôt -> CLV non fiable, exclue de la validation.
...
def prematch(curve, commence):
    pts = sorted((parse(pt[0]), pt[1]) for pt in curve if len(pt) >= 2 and pt[1] and parse(pt[0]))
...
def load_fixtures():
    fix = {}
    # RÉGRESSION CORRIGÉE (16/08/2026) : book_curves.jsonl (fichier unique)
    # n'existe plus depuis la migration en partitions du 14/08 (140,73 Mo ->
    # limite dure GitHub, remplacé par parts/hist_book_*.jsonl). Ce script
    # ouvrait directement BOOK_CURVES en fichier unique -> "introuvable" à
    # chaque run depuis, silencieusement (le workflow sortait tôt sans
    # erreur). iter_hist_lines('book') lit l'ensemble des partitions comme
    # un flux unique, de façon rétrocompatible.
    for line in ov.iter_hist_lines('book'):
        ...
        if len(ph) < 2 or len(pa) < 2:
            continue
        fix.setdefault(x.get('fixture_id') or x.get('uid'), {})[x.get('book')] = {
...
def main():
    if not os.path.exists(BOOK_CURVES) and not os.path.exists('parts'):
        print(f"❌ ni {BOOK_CURVES} ni parts/ introuvables"); return
    fix = load_fixtures()
    ...
    for f in pairs:
        ...
        # Récence du closing sharp : dernier point pré-match -> coup d'envoi. Trop ancien = CLV exclue.
        com = sharp['commence']
        last_sharp_t = max(sharp['home_pts'][-1][0], sharp['away_pts'][-1][0])
        close_mins = round((com - last_sharp_t).total_seconds() / 60) if com else None
        clv_reliable = (close_mins is not None and close_mins <= CLV_CLOSE_MAX_MINS)
        ...
        for side, soft_open, instab, fair_open, fair_close in (
            ...
            flags.append({
                ...
                'clv_close_mins': close_mins, 'clv_reliable': clv_reliable,
            })
    ...
    clv = [r['clv_pct'] for r in flags if r.get('clv_reliable')]
    n_stale = sum(1 for r in flags if not r.get('clv_reliable'))
    out.append(f"Flags : {len(flags)} (sur {len(pairs)} fixtures)")
    out.append(f"Closings fiables ≤{CLV_CLOSE_MAX_MINS}min : {len(clv)} retenus | {n_stale} périmés exclus de la CLV")
    s = stats(clv)
    if s:
        n, m, med, tm, pos, t = s
        out += [
