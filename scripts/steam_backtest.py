...
def load_curves():
    data = {}
    try:
        _lines = ov.open_curves(CURVES)
    except FileNotFoundError as e:
        print(f"❌ {e}"); return data
    for line in _lines:
        line = line.strip()
        if not line: continue
        r = json.loads(line)
        ct = _dt(r.get('commence_time'))
        # PRÉ-MATCH UNIQUEMENT : ~90% des points de book_curves.jsonl sont in-play.
        # Les inclure fausse le backtest (close/t_close deviennent des prix in-play
        # qui encodent le résultat -> look-ahead). On coupe au coup d'envoi.
        def _pre(seq):
