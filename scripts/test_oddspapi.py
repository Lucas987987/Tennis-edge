...
def test():
    ...
    for s in sports:
        ...
        sname = str(s.get('sportName') or s.get('name','')).lower().strip()
        # Tennis EXACT (exclure "table tennis"/"tennis de table")
        if sname == 'tennis':
            tennis_id = sid
            print(f"  Tennis (vrai) trouvé: id={sid}, name={s.get('sportName') or s.get('name')}")
