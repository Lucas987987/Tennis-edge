...
def fetch_csv(repo, filename):
    # 1) CDN raw (public, sans limite d'API) ; 2) repli API contents (avec token)
    raw = f"https://raw.githubusercontent.com/JeffSackmann/{repo}/master/{filename}"
    try:
        req = urllib.request.Request(raw, headers={'User-Agent': 'tennis-edge-bot/1.0'})
        with urllib.request.urlopen(req, timeout=30) as r:
            return r.read().decode('utf-8')
    except Exception as e_raw:
        try:
            api_url = f"https://api.github.com/repos/JeffSackmann/{repo}/contents/{filename}"
            dl = json.loads(github_get(api_url)).get('download_url')
            if not dl:
                print(f"  ⚠️ {filename}: pas de download_url (raw: {e_raw})"); return None
            req = urllib.request.Request(dl, headers={'User-Agent': 'tennis-edge-bot/1.0'})
            with urllib.request.urlopen(req, timeout=30) as r:
                return r.read().decode('utf-8')
        except Exception as e_api:
            print(f"  ⚠️ {filename}: raw={e_raw} | api={e_api}")
            return None
...
def main():
    ...
    # Garde-fou : ne JAMAIS ecraser players_data.json si les fetchs ont echoue.
    if not new_atp and not new_wta:
        print("\n❌ 0 joueur recupere (fetch Sackmann en echec) — players_data.json CONSERVE intact "
              "(pas d'ecrasement). Echec volontaire pour signaler le probleme.")
        raise SystemExit(1)
